...
export function CourseReviews({
  ...
  const { data: myReview } = useQuery({
    queryKey: ["my-review", user?.id, courseId],
  ...
  const save = useMutation({
    mutationFn: () =>
      upsertMyReview({ course_id: courseId, rating, comment: comment || null }),
    onSuccess: () => {
      setShowForm(false);
      qc.invalidateQueries({ queryKey: ["course-reviews", courseId] });
      qc.invalidateQueries({ queryKey: ["my-review", user?.id, courseId] });
  ...
  const remove = useMutation({
    mutationFn: () => deleteMyReview(myReview!.id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["course-reviews", courseId] });
      qc.invalidateQueries({ queryKey: ["my-review", user?.id, courseId] });
      qc.invalidateQueries({ queryKey: ["rating-summary", courseId] });
    ...
    <div className="mt-12 bg-white rounded-xl border border-border p-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        ...
        {isEnrolled && !showForm && (
          ...
          >
            {myReview ? "Editar minha avaliação" : "Avaliar este curso"}
          </button>
        )}
      </div>
