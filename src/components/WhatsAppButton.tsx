...
const DEFAULTS = {
  ...
  whatsapp_phone: "5511994061178",
  whatsapp_message: "Olá Elisa, tudo bem?",
...
export async function fetchWhatsAppConfig(): Promise<Record<Key, string>> {
  const keys = Object.keys(DEFAULTS) as Key[];
  const entries = await Promise.all(
    keys.map(async (k) => {
      try {
        const v = await getSetting(k);
        return [k, v !== null && v !== "" ? v : DEFAULTS[k]] as const;
      } catch {
        return [k, DEFAULTS[k]] as const;
      }
    })
  );
...
const WhatsAppButton = () => {
  ...
  if (cfg.whatsapp_enabled === "false") return null;

  const phone = (cfg.whatsapp_phone || "").replace(/\D/g, "");
  if (!phone) return null;

  const href = `https://wa.me/${phone}${cfg.whatsapp_message ? `?text=${encodeURIComponent(cfg.whatsapp_message)}` : ""}`;
  const isLeft = cfg.whatsapp_position === "left";
